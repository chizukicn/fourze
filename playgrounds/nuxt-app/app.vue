...
<template>
  <div class="px-4">
    <div class=" font-bold py-4 text-2xl text-light-blue-400">
      Image Upload/Load
    </div>
    <div class="flex space-x-4 px-8 items-center">
      ...
      <div>
        <button
          class="bg-light-blue-400 text-white py-1 px-2 hover:bg-opacity-60"
          @click="upload"
        >
          Upload
        </button>
    ...
    <div class="px-8">
      ...
      <div class="flex space-x-4">
        <textarea readonly :value="JSON.stringify(result, null, 4)" class="bg-gray-400 h-100 w-100" />
        <div>
          <div v-show="time" class="text-lg text-light-blue-400">
            loading time:{{ time }}ms
          </div>
          <div v-show="serverDelay > 0" class="text-lg text-light-blue-400">
            server delay:{{ serverDelay }}ms
          </div>
          <div v-show="time - serverDelay > 0" class="text-lg text-light-blue-400">
            response time:{{ time
              - serverDelay }}ms
          </div>
        </div>
